...
- **O1** defaults — **Done (2026-09-13)**: owned-only dataplane and descriptor shadows are on by
  ...
  **runner now sets `auto`** (local demo + k8s manifests; `-Ds2.partition=<scheme>` overrides) and
  `S2_PREFIX_SHARDS` stays off. **O2** k8s resources / `-Xmx` (the worker `JAVA_TOOL_OPTIONS` now also carries
  `-Ds2.prefixSpacePositiveCacheOnly=true` and `-Ds2.partition=auto`); **O3** CI demo matrix — **Done**:
  ...
  **v2 topology correction (full-table propagation closure) is implemented but opt-in and does not
  ...
  (cross-network scale only; per-network unchanged). Left gated, default off; §6.9.
  **O6 residual (role-level scaling), positive but gated off**: `-Ds2.nodeWeightsRoleScale=true`
  ...
  calibrated 3 (s2-fat2-like). Measured-cost imbalance (`scripts/calibrate-weights.py imbalance` over
  O6 main-RIB counts) improves `s2-fat4` W=3 from 1.061 → 1.047 and leaves `s2-fat2` (1.148) and the
  other testbeds unchanged (one assignment changes with no metric change). A single peer coefficient
  cannot do both: `s2-fat2` needs ≥2, `s2-fat4` is best at 0 (`-Ds2.nodeWeightsPeerScale` sweep).
  Kept gated off like v2 pending broader validation; §6.10.
  **O7** docs sync.
