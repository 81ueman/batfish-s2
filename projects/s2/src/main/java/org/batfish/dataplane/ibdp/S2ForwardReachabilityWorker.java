...
/**
 * Forward distributed reachability fixpoint with per-worker-local graph generation.
 *
 * <p>The worker's analysis is built with an {@link OwnedForwardingAnalysis}, so it only contains
 * transitions whose source is a locally-owned switch. Forward propagation from local ingress states
 * therefore only needs local edges; when a transition targets another worker's state, the BDD is
 * shipped there over {@link S2BddSidecar}. No remote edges need to be pulled (unlike backward).
 */
public final class S2ForwardReachabilityWorker {
  ...
  public S2ForwardReachabilityWorker(
      int id,
  ...
  /** Number of edges this worker generated (for scale reporting). */
    ...
    while (!_worklist.isEmpty()) {
      ...
      for (Map.Entry<StateExpr, Transition> edge : _edges.row(state).entrySet()) {
        ...
        if (owner(post) == _id) {
          ...
          _pending.computeIfAbsent(owner(post), k -> new ArrayList<>()).add(new Outbound(post, out));
        }
      }
    }
  }
